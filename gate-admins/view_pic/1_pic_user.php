...
if (isset($_POST['cmd'])) {
	switch ($_POST['cmd']) {
		...
		case 'Update':
			...
			$role = $_POST['role'];
			if ($role != 'role') {
				$par = updateUser_1($id, $name, $dob, $pob, $address, $phone, $email, $role, $uname);
				if ($par == true) {
					$js = '<script>alert("User '.$name.' was updated.");</script>';
					echo($js);
				}
			} else {
				$par = updateUser_2($id, $name, $dob, $pob, $address, $phone, $email, $uname);
				if ($par == true) {
					$js = '<script>alert("User ' . $name . ' was updated.");</script>';
					echo($js);
				}
