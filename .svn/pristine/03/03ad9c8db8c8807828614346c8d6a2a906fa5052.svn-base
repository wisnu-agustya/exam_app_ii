...
/*By Prog*/	} else if ($by=='Program') {
		...
		case 'exam':
			...
			foreach ($exam as $key => $value) {
				...
				$header=array('No','ID Student','Student Name','Start Exam','End Exam','True Answer','Percentage','Status');
				echo "
				...
				while ($row = mysqli_fetch_row($result)) {
					...
					$field[$value][$no] = array(' '.$row[8],$row[1],$row[2],$row[3],$row[4],number_format($row[6],2).'%',$status2);
					echo "
						<tr>
							<td>$no</td>
							<td>".$row[8]."</td>
