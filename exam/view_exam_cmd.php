...
// tombol navigasi 
		echo "<form id=\"navigation\" action=\"\" method=\"POST\">
	         ...
	         ";
        if ($nomorSoal > 1){
			//echo "<button class = \"btn btn-sm btn-success\" onclick=\"formSubmitPrev()\">Prev</button>&nbsp";
			echo "<input class = \"btn btn-sm btn-success\" value=\"Prev\" onclick=\"formSubmitPrev()\">&nbsp";
		}
		if ($nomorSoal < $jumlahSoal){
			//echo "<button class = \"btn btn-sm btn-success\" onclick=\"formSubmitNext()\">Next</button>";
			echo "<input class = \"btn btn-sm btn-success\" value=\"Next\" onclick=\"formSubmitNext()\">";
		} 
		if ($row[10]=='True') {
			echo "&nbsp<input class = \"btn btn-sm btn-warning\" id='btn-unmark' value=\"Unmark\" onclick=\"formSubmitUnmark()\" readonly> &nbsp";
		}else{
			echo "&nbsp<input class = \"btn btn-sm btn-warning\" id='btn-mark' value=\"Mark\" onclick=\"formSubmitMark()\" readonly> &nbsp";
		}
      echo "</form>";
 		if ($nomorSoal == $jumlahSoal){
			echo "&nbsp<input class = \"btn btn-sm btn-primary\" value=\"Review\" onclick=\"formSubmitReview()\"> &nbsp";
//			echo "<a href=\"review.php\"><button class =\"btn btn-sm btn-primary\" value=\"review\"> Review </button></a>	";
		} 
        echo "
        		</div>
        		</div>
       ...
       <script>
		...
		function formSubmitMark() {
			if (document.getElementById('btn-mark').value=='Mark') {
				document.getElementById('mark').value='true';
				document.getElementById('btn-mark').value='Unmark';
			}else{
				document.getElementById('mark').value='false';
				document.getElementById('btn-mark').value='Mark';
			}
		    //document.getElementById('navigation').submit();
		}
		function formSubmitUnmark() {
			if (document.getElementById('btn-unmark').value=='Mark') {
				document.getElementById('mark').value='true';
				document.getElementById('btn-unmark').value='Unmark';
			}else{
				document.getElementById('mark').value='false';
				document.getElementById('btn-unmark').value='Mark';
			}
			//document.getElementById('navigation').submit();
		}
...
function endSession($id_peserta,$exam_group){
	...
	//echo $sql;
	mysqli_query($GLOBALS['link'],$sql) or  die(mysqli_error($GLOBALS['link']));
	$sql="delete from exam_run_quest where id_student='".$id_peserta."' and group_name ='$exam_group'";
